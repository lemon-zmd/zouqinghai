package com.zouqinghai.activity.waterfall;
...
import com.zouqinghai.activity.waterfall.LazyScrollView.OnScrollListener;
import com.zouqinghai.activity.waterfall.widget.FlowView;
...
//    private Context context;
    ...
    public WaterFallUtil(Activity act){
//        this.context = ctx;
        this.activity = act;
    }
    public void create(int waterFallScrollidInR, int waterFallContaineridInR) {
        ...
        initLayout(waterFallScrollidInR, waterFallContaineridInR);

    }

    private void initLayout(int waterfallScrollId, int waterFallContainerId) {
        waterfall_scroll = (LazyScrollView) activity.findViewById(waterfallScrollId);
                        ...
                        for (int k = 0; k < column_count; k++) {
                            ...
                            if (pin_mark[k].get(Math.min(bottomIndex[k] + 1, lineIndex[k])) <= t + 3 * scroll_height) {// 最底部的图片位置小于当前t+3*屏幕高度
                                ...
                                ((FlowView) waterfall_items.get(k).getChildAt(
                                        Math.min(1 + bottomIndex[k], lineIndex[k]))).Reload();

                                bottomIndex[k] = Math.min(1 + bottomIndex[k], lineIndex[k]);

                            }
                            Log.d("MainActivity", "headIndex:" + topIndex[k] + "  footIndex:" + bottomIndex[k]
                                    + "  headHeight:" + pin_mark[k].get(topIndex[k]));
        ...
        waterfall_container = (LinearLayout) activity.findViewById(waterFallContainerId);
        handler = new Handler() {
