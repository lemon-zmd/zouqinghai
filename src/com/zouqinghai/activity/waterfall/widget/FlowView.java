package com.zouqinghai.activity.waterfall.widget;

import java.io.BufferedInputStream;
import java.io.IOException;

import com.zouqinghai.activity.waterfall.Constants;
...
public class FlowView extends ImageView implements View.OnClickListener, View.OnLongClickListener {
    ...
    class ReloadImageThread extends Thread {
        ...
        public void run() {
            ...
            try {
                buf = new BufferedInputStream(context.getAssets().open(getFileName()));
            ...
            ((Activity) context).runOnUiThread(new Runnable() {
                public void run() {
                    if (bitmap != null) {// 此处在线程过多时可能为null
                        ...
                        Message m = h.obtainMessage(Constants.HANDLER_WHAT, width, layoutHeight, FlowView.this);
                        h.sendMessage(m);
                    }
                }
            });
